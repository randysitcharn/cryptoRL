...
def create_test_env_with_prices(prices: list, window_size: int = 10, **kwargs):
    """Create BatchCryptoEnv with specific price series. kwargs passed to BatchCryptoEnv."""
...
def _run_policy_steps(env, position: float, steps: int):
    """Run `steps` steps with constant action [position]. Returns list of step rewards."""
    env.gym_reset()
    rewards = []
    action = np.array([position], dtype=np.float32)
    for _ in range(steps):
        _, reward, term, trunc, _ = env.gym_step(action)
...
def test_dsr_get_global_metrics():
    """Test: get_global_metrics contains DSR keys (reward/dsr_raw, reward/dsr_A, reward/dsr_B)."""
    print("\nTest 5: DSR get_global_metrics...")

    prices = [100.0 + 0.001 * i + 0.001 * (i % 3) for i in range(150)]
    env, tmp_path = create_test_env_with_prices(
        prices, window_size=10,
        dsr_eta=0.02, dsr_warmup_steps=10,
    )
    try:
        env.gym_reset()
        for _ in range(15):
        ...
        for key in ("reward/dsr_raw", "reward/dsr_A", "reward/dsr_B"):
            assert key in metrics, f"Missing '{key}' in get_global_metrics"
        print(f"  Keys: reward/dsr_raw, reward/dsr_A, reward/dsr_B present")
        print("  PASSED!")
    finally:
        cleanup_env(tmp_path)
...
def test_dsr_policy_discrimination():
    """Test: DSR yields significantly different cumulative rewards for B&H (+1.0) vs small position (+0.05).
    With r_alpha in flat market, B&H and Cash are similar; with DSR they must differ."""
    print("\nTest 6: DSR policy discrimination...")

    n = 130
    # Mild trend so B&H vs +0.05 have different return streams; DSR can discriminate
    prices = [100.0 + 0.002 * i for i in range(n)]
    env, tmp_path = create_test_env_with_prices(
        prices, window_size=10,
        dsr_eta=0.02, dsr_warmup_steps=10,
    )
    try:
        steps = 100
        rewards_fixed = _run_policy_steps(env, 0.05, steps)
        rewards_bh = _run_policy_steps(env, 1.0, steps)
        mean_fixed = np.mean(rewards_fixed)
        mean_bh = np.mean(rewards_bh)
        diff = abs(mean_bh - mean_fixed)
        threshold = max(0.1, 0.5 * np.std(rewards_fixed)) if np.std(rewards_fixed) > 1e-8 else 0.1
        assert diff > threshold, (
            f"DSR should discriminate B&H vs small position: |mean_bh - mean_fixed)|={diff:.6f} <= {threshold:.6f}"
        )
        print(f"  |mean(B&H) - mean(+0.05)| = {diff:.6f} > {threshold:.6f}")
        print("  PASSED!")
    finally:
        cleanup_env(tmp_path)
