...
class FoundationFeatureExtractor(BaseFeaturesExtractor):
    ...
    Architecture:
    - Full market observation (all 55 features) → MAE Encoder (frozen) → embeddings (B, 64, 128)
    ...
    Critical: The MAE receives the FULL observation (55 features) to match the pre-trained
    weights. HMM features are extracted for FiLM modulation but the MAE sees all columns.
    ...
    def __init__(
        ...
        # Create CryptoMAE encoder with FULL input dimension (includes HMM features)
        # The encoder was pre-trained with all 55 features - do NOT reduce input_dim
        self.mae = CryptoMAE(
            input_dim=self.n_features,  # Must match pre-trained dimension (55)
            d_model=d_model,
            n_heads=n_heads,
            n_layers=n_layers,
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        Args:
            observations: Dict with:
                - "market": (batch, seq_len, n_features) market data
                  - Full observation (all 55 features) goes to MAE
                  - HMM context (last 5 cols) is extracted for FiLM modulation
        ...
        # 1. Encode FULL market observation using pretrained MAE encoder (frozen)
        # The encoder was trained with all 55 features - do NOT split the input
        ...
        # 2. Extract HMM context for FiLM modulation (last 5 cols at last timestep)
        # This is extracted AFTER encoding, HMM features are also seen by MAE
        if self.use_film:
